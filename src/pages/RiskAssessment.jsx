...
//         const allData = extracted[reportId] || [];
//         const diabetesData = allData.filter(d =>
//           d.param && (
//             d.param.toLowerCase().includes('glucose') ||
//             d.param.toLowerCase().includes('sugar') ||
//             d.param.toLowerCase().includes('hba1c') ||
//             d.param.toLowerCase().includes('glycated')
//           )
//         );
//         setMedicalData(diabetesData);
//       } else {
//         navigate('/reports');
//       }
//       setLoading(false);
...
//       const isHigh = data.value > data.normal_range.high;
//       const isBorderline = !isHigh && data.value > data.normal_range.high * 0.9;
//       const status = isHigh ? 'Critical' : (isBorderline ? 'Borderline' : 'Normal');
//       const score = status === 'Critical' ? 85 : (status === 'Borderline' ? 58 : 12);
...
//         score: score,
//         status: status,
//         description: `Your ${data.param.toLowerCase()} level is ${data.value} ${data.unit}. ${isHigh ? 'Elevated levels detected.' : (data.value < data.normal_range.low ? 'Low levels detected.' : 'Within normal range.')}`,
//         recommendations: [
//           `Monitor ${data.param.toLowerCase()} levels regularly`,
//           `Focus on a low-glycemic index diet`,
//           `Consult your endocrinologist or primary care physician`
//         ],
