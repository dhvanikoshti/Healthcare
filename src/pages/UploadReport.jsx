    ...
    <Layout>
      ...
      {viewReport && (
        ...
        >
          {/* Header bar */}
          <div
            className="flex items-center justify-between px-5 py-3 bg-white/10 backdrop-blur border-b border-white/10"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center gap-3 min-w-0">
              {getFileIcon(viewReport.type)}
              <div className="min-w-0">
                <p className="text-white font-semibold text-sm truncate max-w-xs md:max-w-lg">{viewReport.name}</p>
                <p className="text-white/60 text-xs">{viewReport.date} • {viewReport.size}</p>
              </div>
            </div>
